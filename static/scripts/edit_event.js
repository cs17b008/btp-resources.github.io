...
function StartEditEvent(anno_id,event) {
  ...
  if((IsUserAnonymous() || (!IsCreator(LMgetObjectField(LM_xml, anno_id, 'username')))) && (!IsUserAdmin()) && (anno_id<num_orig_anno) && !action_RenameExistingObjects && !action_ModifyControlExistingObjects && !action_DeleteExistingObjects) {
    ...
    /* ----------------------------------------------------- */
  /**
    * Allow multiple users to annotate same region
    * 
    * @since      26-05-2020
  ...
  */
    ...
    var anno = main_canvas.GetAnnotation(anno_id);
    ...
    if(LMgetObjectField(LM_xml, anno.anno_id, 'verified')) {
      ...
      // Popup edit bubble:
      WriteLogMsg('*Opened_New_Edit_Popup');
      mkNewEditPopup(pt[0],pt[1],anno);
...
/**
  * This function is called when the edit event for multiple users is finished.  
  * It can be triggered when the user 
  * (1) clicks the close edit bubble button, 
  * (2) zooms, (3) submits an object label in the popup bubble, 
  * (4) presses the cancel button in the popup bubble, (5) clicks the 
  * object in the object list, (6) presses the ESC key.
  * 
  * @since      26-05-2020
  * @author     Reena Deshmukh <cs16b029@iittp.ac.in>
...
function StopNewEditEvent() {
  ...
  // Write logfile message:
  WriteLogMsg('*Closed_New_Edit_Popup');
  ...
  console.log('LabelMe: Stopped new edit event.');
}
