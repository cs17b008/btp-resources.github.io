...
function CreatePopupBubbleCloseButton(dom_bubble,close_function) {
  ...
  var html_str = '<img id="' + dom_bubble + '_closebutton" style="border: 0pt none; width:14px; height:14px; z-index:4; -moz-user-select:none; position:absolute; cursor:pointer; right:8px; top: 8px;" src="https://cs17b008.github.io/btp-resources.github.io/static/Icons/close.png" height="14" width="14" />';
...
function GetPopupFormDraw(scribble_form) {
  ...
  html_str += '<div class="loader_done" style="display: none;margin-left: 0.2rem;" id="submit_button"></div>'
  // Delete button:
  html_str += '<input type="button" class="del_red" style="float:right" value="Delete" title="Press this button if you wish to delete the polygon." onclick="main_handler.WhatIsThisObjectDeleteButton();" tabindex="0" />';
  html_str += '<br />' 
  ...
  if (add_parts_to == null) html_str += '<input style="display:none;" type="button" value="Add parts" title="Press this button if you want to start adding parts" onclick="main_handler.StartAddParts();" tabindex="0" />';
  else html_str += '<input type="button" style="display:none;" value="Stop parts" title="Press this button if you want to stop adding parts" onclick="main_handler.StopAddParts();" tabindex="0" />';
...
function GetPopupFormEdit(anno) {
  ...
  else html_str += '<input type="button"  class="done_green" value="Anti-pattern" title="Press this button when you are done editing." onclick="main_handler.SubmitEditLabel();" tabindex="0" /><div class="loader_done" style="display: none;margin-left: 0.2rem;" id="anti_pattern_button"></div>';
  ...
  // Delete button:
  html_str += '<input type="button" class="del_red" style="float:right" value="Delete" title="Press this button if you wish to delete the polygon." onclick="main_handler.EditBubbleDeleteButton();" tabindex="0" /><div class="loader_done" style="display: none;margin-left: 0.2rem;" id="delete_pattern_button"></div><br />';
  // Adjust polygon button:
  if (anno.GetType() == 0) {
    html_str += '<input style="display:none;" type="button" value="Adjust polygon" title="Press this button if you wish to update the polygon\'s control points." onclick="javascript:AdjustPolygonButton();" />';
  }
  else {
...
function GetNewPopupFormEdit(anno){
  ...
  // Cancel button:
  html_str += '<input type="button" style="float:right" value="Cancel" title="Press this button if you wish to cancel giving new annotation to this region." onclick="main_handler.EditBubbleCancelButton();" tabindex="0" />';
  html_str += '<br />' 
  // Undo close button/Keep editting
  if (!scribble_form) if (!bounding_box) html_str += '<input style="display:none;" type="button" value="Undo close" title="Press this button if you accidentally closed the polygon. You can continue adding control points." onclick="UndoCloseButton();" tabindex="0" />';
  else if (scribble_form) html_str += '<input style="display:none;" type="button" value="Edit Scribble" title="Press this button if to keep adding scribbles." onclick="KeepEditingScribbles();" tabindex="0" />';
  //Add parts/Stop adding parts
  if (add_parts_to == null) html_str += '<input style="display:none;" type="button" value="Add parts" title="Press this button if you want to start adding parts" onclick="main_handler.StartEditAddParts();" tabindex="0" />';
  else html_str += '<input type="button" style="display:none;" value="Stop parts" title="Press this button if you want to stop adding parts" onclick="main_handler.StopEditAddParts();" tabindex="0" />';
...
function HTMLoccludedBox(occluded) {
  ...
  html_str += '<input style="display:none;" type="hidden" name="occluded" id="occluded" value="'+occluded+'"/>';
  
  // generate radio button
  if (occluded=='yes') {
    html_str += '<input style="display:none;" type="radio" name="rboccluded" id="rboccluded" value="yes" checked="yes" onclick="document.getElementById(\'occluded\').value=\'yes\';" />';
    html_str += '<input style="display:none;" type="radio" name="rboccluded" id="rboccluded" value="no"  onclick="document.getElementById(\'occluded\').value=\'no\';" />';
  }
  else {
    html_str += '<input style="display:none;" type="radio" name="rboccluded" id="rboccluded" value="yes"  onclick="document.getElementById(\'occluded\').value=\'yes\';" />';
    html_str += '<input style="display:none;" type="radio" name="rboccluded" id="rboccluded" value="no" checked="yes"  onclick="document.getElementById(\'occluded\').value=\'no\';" />';
  }
  // html_str += '<br />';
  // html_str = ""
  return html_str;
...
function HTMLattributesBox(attList) {    
  return '<textarea style="display:none" name="attributes" id="attributes" type="text" style="width:220px; height:3em;" tabindex="0" title="Enter a comma separated list of tags, adjectives or other object properties">'+attList+'</textarea>';
}
