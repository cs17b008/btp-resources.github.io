...
function LMgetObjectField(xml,ind_object, name, frame) {
	...
	/**
	 * Fetch all the tags corresponding to given anno_id and return
	 */
	if(name=="attributes")
	{
		var tags = obj.children("taglist").children("tag");
		
			var taglist = Array(tags.length);
				for (var ii=0; ii < tags.length; ii++){
					
					taglist[ii] = tags.eq(ii).text();
				}
				return taglist;
...
function LMsetObjectField(xml, ind_object, name, value){
	var obj = $(xml).children("annotation").children("object").eq(ind_object);
	if (name == 'name' || name == 'automatic' || name == 'occluded' || name == 'deleted' || name == 'id'){
	...
	/**
	 * Set taglist corresponding to given Anno_id
	 */
	if (name == 'attributes')
	{
		orig_taglist_length = obj.children("taglist").children("tag").length;
		var j=0;
		for (var ii = 0; ii < value.length; ii++){
			if(value[ii].length>0)
			{
				if(j<=orig_taglist_length-1)
				{
					obj.children("taglist").children("tag").eq(ii).text(value[ii]);
				}
				else
				{
					obj.children("taglist").append($("<tag>"+value[ii]+"</tag>"));
				}
				j++;
			}
		}
		/**
		 * This triggers when tags are removed.
		 */
		if(j<=orig_taglist_length)
		{
			for(var i=orig_taglist_length-1; i>=j;i--)
			{
				obj.children("taglist").children("tag").eq(i).remove();
			}
		}
	}
