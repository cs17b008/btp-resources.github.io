    ...
    if(video_mode) {
      ...
      $.getScript("annotationTools/js/video.js", function(){
        ...
        function main_media_onload_helper(){
          ...
          anno_file = 'VLMAnnotations/' + anno_file + '.xml' 
      ...
      function main_media_onload_helper() {
          ...
          var anno_file = main_media.GetFileInfo().GetFullName();
          console.log('kowndinya-startup.js: ' + anno_file)
...
function SetAllAnnotationsArray() {
  ...
  // //Setting default value of global variable tess_anno
  // tess_anno = 0;
  ...
  for(var pp = 0; pp < num_obj; pp++) {
    ...
    //Set the global variable tess_anno = 1 if annotations by tesseract already exist
    // if(curr_obj.children("polygon").length == 0) { // Segmentation
    //   if(curr_obj.children("segm").children("username") == "tesseract") {
    //     tess_anno = 1;
    //   }
    // }
    // else if(curr_obj.children("polygon").children("username") == "tesseract") {
    //    tess_anno = 1;
    // } 
...
function InitializeAnnotationTools(tag_button, tag_canvas){
    ...
    if (document.getElementById("polygonDiv")){
      ...
      document.getElementById("polygonDiv").setAttribute('style', 'border: none');
      document.getElementById("polygonDiv").setAttribute('style', 'margin-top: 9rem;');
...
function SetDrawingMode(mode){
    ...
    if (mode == 0){
        ...
        document.getElementById("polygonDiv").setAttribute('style', 'border: none');
        document.getElementById("polygonDiv").setAttribute('style', 'margin-top: 5rem;');
